...
#Region "   Eventos"
    ...
    Private Sub frmVentas_Peron_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SQL = "exec [spVentas_Select_ALL] @Eliminado = 0"
    ...
    Private Sub chkEliminado_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkEliminado.CheckedChanged
        If bolModo = False Then
            ...
            If chkEliminado.Checked = True Then
                ...
                SQL = "exec [spVentas_Select_ALL] @Eliminado = 1"
    ...
    Private Sub txtCantidad_KeyDown(sender As Object, e As KeyEventArgs) Handles txtCantidad.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            For i = 0 To grdItems.RowCount - 1
                If cmbProducto.Text = grdItems.Rows(i).Cells(0).Value Then
            ...
            grdItems.Rows.Add(cmbProducto.Text, txtCantidad.Text, txtPrecioVta.Text, txtSubtotal.Text, 0, 0, txtSubtotal.Text, 0, 0, cmbProducto.SelectedValue, txtIdUnidad.Text)
    ...
    Private Sub grditems_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles grdItems.CellClick
        If e.ColumnIndex = 7 Then 'Marcar llegada
    ...
    Private Sub grdItems_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdItems.CellEndEdit
        ...
        Try
            ...
            If e.ColumnIndex = 4 Then
                ...
                grdItems.Rows(cell.RowIndex).Cells(5).Value = grdItems.Rows(cell.RowIndex).Cells(1).Value * grdItems.Rows(cell.RowIndex).Cells(4).Value
                grdItems.Rows(cell.RowIndex).Cells(6).Value = grdItems.Rows(cell.RowIndex).Cells(3).Value - grdItems.Rows(cell.RowIndex).Cells(5).Value
...
#Region "   Procedimientos"
    ...
    Private Sub LlenarGridItems()
        ...
        Try
            ...
            sqltxt2 = "exec [spVentas_Det_Select_By_IdVenta] @idventa = " & txtID.Text
            ...
            For i = 0 To dt.Rows.Count - 1
                grdItems.Rows.Add(dt.Rows(i)(0).ToString(), dt.Rows(i)(1).ToString(), dt.Rows(i)(2).ToString(), dt.Rows(i)(3).ToString(), dt.Rows(i)(4).ToString(), dt.Rows(i)(5).ToString(), dt.Rows(i)(6).ToString(), 0, dt.Rows(i)(7).ToString(), dt.Rows(i)(8).ToString(), dt.Rows(i)(9).ToString())
            Next
    ...
    Private Function AgregarActualizar_Registro(ByVal PagoContado As Boolean) As Integer
        ...
        Try
            ...
            Try
                ...
                Try
                    If bolModo = True Then
                        SqlHelper.ExecuteNonQuery(tran, CommandType.StoredProcedure, "[spVentas_Insert]", param_id, _
    ...
    Private Function AgregarActualizar_Registro_Items() As Integer
        ...
        Try
            ...
            For i = 0 To grdItems.Rows.Count - 1
                ...
                param_idProducto.Value = grdItems.Rows(i).Cells(9).Value
                ...
                param_idUnidad.Value = grdItems.Rows(i).Cells(10).Value
                ...
                param_Producto.Value = grdItems.Rows(i).Cells(0).Value
                ...
                param_Preciouni.Value = grdItems.Rows(i).Cells(2).Value
                ...
                param_Subtotal.Precision = 18
                param_Subtotal.Scale = 2
                param_Subtotal.Value = grdItems.Rows(i).Cells(3).Value
                param_Subtotal.Direction = ParameterDirection.Input
                ...
                param_DescxUnidad.Value = grdItems.Rows(i).Cells(4).Value
                ...
                param_TotalDesc.Value = grdItems.Rows(i).Cells(5).Value
                ...
                param_SubtotalFinal.Value = grdItems.Rows(i).Cells(6).Value
                ...
                Try
                    SqlHelper.ExecuteNonQuery(tran, CommandType.StoredProcedure, "spVentas_Det_Insert", _
                                              param_IdVenta, param_IdAlmacen, param_idProducto, param_idUnidad, param_Producto, _
                                              param_cantidad, param_Preciouni, param_Subtotal, param_DescxUnidad, _
                    ...
                    'If MDIPrincipal.NoActualizar = False And MDIPrincipal.sucursal.Contains("PERON") Then 'Not SystemInformation.ComputerName.ToString.ToUpper = "SAMBA-PC" Then
                    Try
                        ...
                        sqlstring = "exec " & NameSP_spStockInsert & " '" & grdItems.Rows(i).Cells(9).Value & "', '" & _
                        ...
                        If tranWEB.Sql_Get_Value(sqlstring) > 0 Then
                            ds_Empresa = SqlHelper.ExecuteDataset(tran, CommandType.Text, "UPDATE " & NameTable_StockMov & " SET ActualizadoWEB = 1 WHERE id = " & IdStockMov)
                            ds_Empresa.Dispose()
                        End If
        ...
        Try
            ...
            Try
                ...
                Try
                    ...
                    SqlHelper.ExecuteNonQuery(tran, CommandType.StoredProcedure, "spVentas_Delete_All", param_id, param_res)
                    res = param_res.Value
...
#End Region
    ...
    Private Sub cmbProducto_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbProducto.SelectedValueChanged
        ...
        If band = 1 Then
            ...
            If MDIPrincipal.sucursal.Contains("PERON") Then
                ds_Empresa = SqlHelper.ExecuteDataset(ConnStringSEI, CommandType.Text, "SELECT PrecioPeron, m.IdUnidad, s.qty,m.Codigo,m.Nombre,s.IDAlmacen,m.Minimo,PrecioMayoristaPeron " & _
            ...
            Else
                ds_Empresa = SqlHelper.ExecuteDataset(ConnStringSEI, CommandType.Text, "SELECT PrecioCosto, m.IdUnidad, s.qty,m.Codigo,m.Nombre,s.IDAlmacen,m.Minimo,PrecioMayorista " & _
                                  " FROM Materiales m JOIN Stock s ON s.idmaterial = m.Codigo " & _
                                  " where m.Codigo = '" & cmbProducto.SelectedValue & "' AND s.idalmacen = " & Utiles.numero_almacen)
            End If
            ...
            Try
                If chkPrecioMayorista.Checked = False Then
                    txtPrecioVta.Text = Math.Round(ds_Empresa.Tables(0).Rows(0)(0), 2)
                Else
                ...
                idproducto_unitario = ds_Empresa.Tables(0).Rows(0)(3)
                producto_unitario = ds_Empresa.Tables(0).Rows(0)(4)
                almacen_unitario = ds_Empresa.Tables(0).Rows(0)(5)
                'COMPARO EL STOCK QUE HAY DEL PRODUCTO PARA SABER SI ESTA POR DEBAJO DEL MINIMO 
                If (ds_Empresa.Tables(0).Rows(0)(6)).ToString <> "" Then
                    If CDbl(stock_unitario) > CDbl(ds_Empresa.Tables(0).Rows(0)(6)) Then
    ...
    Private Sub CalcularSubtotal()
        ...
        For i = 0 To grdItems.Rows.Count - 1
            Subtotal = Subtotal + grdItems.Rows(i).Cells(6).Value
        Next
